...
date: 2023-08-07T19:00:00Z
lastmod: '2023-08-07'
tags: ['next-js', 'github', 'netlify']
draft: false
summary: 'Deploying a Next.js App Router application to Netlify using GitHub Actions'
layout: PostSimple
---
...
[github-repository]: [https://github.com/willsather/website]
[sather-website]: [https://sather.ws]
...
You can view the deployed application at [sather.ws][sather-website] and the GitHub repository at [GitHub.com][github-repository].
...
```
🗒️ Note: we have jobs for building, testing, and deploying our application
```
...
jobs:
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy
        run: netlify deploy --site ${{ secrets.NETLIFY_DEV_SITE_ID }} --auth ${{ secrets.NETLIFY_AUTH_TOKEN }} --dir=.next --build --prod --message "Deploy - GitHub Actions"
